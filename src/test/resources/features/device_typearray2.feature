Feature: Devices array with allowed types
  ...
  Scenario: NEGATIVE - Device type value is missing for 1 device
    When the device API returns JSON:
      """
      { "devices": [
          { "id": "d1", "type": "A" },
      ...
      """
    Then the match should fail with message "the 'type' value does not match any value in [A, B, C] for item: {\"id\":\"d2\"}" when matched against the device golden "device_typearray_golden.json"
